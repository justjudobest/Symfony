...
class DefaultController extends AbstractController
{
    ...
    /**
     * @Route("/product/{product}/{childProduct}", name="product_detail_children", methods={"GET"})
     * @param Product $product
    ...
    /**
     * @Route("/product/{product}/{childProduct}", name="product_detail_children_js", methods={"POST"} )
     * @param Product $product
     * @param Product $childProduct
     * @param ProductRepository $productRepository
     * @return Response
     * @throws NonUniqueResultException
     */
    public function productViewChildrenJS(Product $product, Product $childProduct, ProductRepository $productRepository): JsonResponse
    {
        $attribute = $productRepository->ProductСommodity($childProduct->getId());
        $children = $productRepository->findChildren($product->getId());
        $images = $product->getImagesProduct();
        $image = "";
        if (count($images ) > 0) {
            $image = $images[1]->getUrl();
        }
        return $this->json([
            'attributes' => $attribute,
                 'product' => [
